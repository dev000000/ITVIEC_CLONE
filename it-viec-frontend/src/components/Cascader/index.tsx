...
interface CascaderProps {
  menuItems: CascaderMenuItems;
  type?: "small" | "large";
}

function Cascader({ menuItems, type }: CascaderProps) {
        ...
        <div className="cascader__dropdown-content">
          ...
          <div className="cascader__dropdown-right">
            {menuItems.items.map((item) => (
              <div
                key={item.id}
                className={`cascader__dropdown-child2 ${activeChild === item.id ? "active" : ""
                  }`}
              >
                <Row>
            ...
            {menuItems.items.map((item) => (
              <div
                ...
                className={`cascader__dropdown-viewmore ${activeChild === item.id && item.viewmore.active
                    ? "active"
                    : ""
                  }`}
              >
